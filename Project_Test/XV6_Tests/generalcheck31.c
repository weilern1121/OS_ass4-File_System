...
int main(int argc, char *argv[]){
    ...
    getProcNameFile(theFile4);
    getProcStatusFile(theFile5);
    ...
    if((pid = fork()) == 0){
      ...
      createAndOpenFileInBlocks(theFile3, NDIRECT + 60, 7);
      open(theFile4, O_RDONLY);
      open(theFile5, O_RDONLY);

      open(theDir1,O_RDONLY);
      open(theDir2,O_RDONLY);
      createAndOpenDir(theDir3);
      createAndOpenFileInBlocks(theFile6, NDIRECT + 100, 3);
    ...
    for(int i = 0;i < 10;i++){
      ...
      assertFileInLSNDparsingData(theFile4, 2);
      assertFileInLSNDparsingData(theFile5, 2);
      assertFileInLSNDparsingData(theFile6, 0);

      assertFileInLSNDparsingData("/console",1);
      assertFileInLSNDparsingData("/proc",2);

...
void assertCantWriteToFile(char *name);
void assertCantWriteToProcDirNameFile();
void assertCantWriteToProcDirStatusFile();
void assertCantWriteToDirFiles(char *path);
...
int main(int argc, char *argv[]){
    ...
    assertCantWriteToFile("/proc");
    assertCantWriteToFile("/proc/ideinfo");
    assertCantWriteToFile("/proc/filestat");
    assertCantWriteToFile("/proc/inodeinfo");

    assertCantWriteToProcDirNameFile();
    assertCantWriteToProcDirStatusFile();

    assertCantWriteToDirFiles("/proc/inodeinfo");
...
void assertCantWriteToFile(char *name){
  int fd;
  char *buf = "thingsToWrite";

  if((fd = open(name, O_RDWR)) < 0){
      printf(1,"error opening %s file !!!\n",name);
      exit();
  }

  if(write(fd, buf, strlen(buf) + 1) >= 0){
    printf(1,"error file write was a success where a failure was expected\n");
    exit();
  }

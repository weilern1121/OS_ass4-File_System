...
void executeSubProcess(int i, char *name);
void printProcName(int pid,void (*readProcessNameFile)(char *name, int bytesPerRead));
void assertFileCantBeRead(char *name, int bytesPerRead);
void printProcDir(int pid, void (*readProcessDir)(char *name));
void assertDirCantBeRead(char *name);
...
int main(int argc, char *argv[]){
    ...
    for(int j = 0;j < 40;j++){
      for(int i = 0;i < NELEM(pids);i++){
        executeSubProcess(i, "sleep");
      }

      for(int i = 0;i < NELEM(pids);i++){
        printProcDir(pids[i],readDir);
        printProcName(pids[i],readFile);
      }

      for(int i = 0;i < NELEM(pids);i++){
        kill(pids[i]);
        wait();
        printProcDir(pids[i],assertDirCantBeRead);
        printProcName(pids[i],assertFileCantBeRead);
      }
...
void assertFileCantBeRead(char *name, int bytesPerRead){
  ...
  if((fd = open(name, O_RDONLY)) >= 0){
      printf(1,"error process file can be read after its has exited !!!\n");
      exit();
  }
...
void assertDirCantBeRead(char *name){
  ...
  if((fd = open(name, O_RDONLY)) >= 0){
      printf(1,"error process dir can be read after its has exited !!!\n");
      exit();
  }

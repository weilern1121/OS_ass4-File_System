...
int main(int argc, char *argv[]){
    ...
    if((pid = fork()) == 0){
      createAndOpenFileInBlocks(theFile1, 4, 3);
      createAndOpenFileInBlocks(theFile2, 8, 5);
      createAndOpenFileInBlocks(theFile3, NDIRECT + 5, 7);

      createAndOpenDir(theDir1);
      createAndOpenDir(theDir2);
    ...
    assertFileInLSNDparsingData(theDir1,0);
    assertFileInLSNDparsingData(theDir2,0);
